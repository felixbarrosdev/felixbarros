...
function felixbarros_comment_callback( $comment, $args, $depth ) {
	...
	?>
	<div <?php comment_class( 'bg-gray-50 rounded-lg p-6 border-l-4 border-primary' ); ?> id="comment-<?php comment_ID(); ?>">
		<div class="flex items-start space-x-4">
			...
			<div class="flex-1 min-w-0">
				<div class="flex items-center space-x-2 mb-2">
					<h4 class="font-title font-semibold text-dark">
						<?php echo get_comment_author(); ?>
					</h4>
					<time class="text-sm text-gray-600" datetime="<?php comment_time( 'c' ); ?>">
						<?php comment_time( 'j F Y \a \l\a\s G:i' ); ?>
					</time>
				</div>
				
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<p class="text-sm text-amber-600 bg-amber-50 px-3 py-2 rounded mb-3">
				...
				<div class="mt-3 flex items-center space-x-4">
					...
					?>
					<?php edit_comment_link( 'Editar', '<span class="text-gray-500 text-sm">', '</span>' ); ?>
				</div>
			</div>
		</div>
	</div>
...
function felixbarros_comment_form_args() {
	...
	return array(
		...
		'comment_field'       => '<div class="mb-4"><label for="comment" class="block text-sm font-medium text-dark mb-2">Comentario *</label><textarea id="comment" name="comment" rows="6" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors resize-none" placeholder="Escribe tu comentario aquí..." required></textarea></div>',
		'fields'              => array(
			'author' => '<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"><div><label for="author" class="block text-sm font-medium text-dark mb-2">Nombre' . ( $req ? ' *' : '' ) . '</label><input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors" placeholder="Tu nombre"' . $aria_req . ' /></div>',
			'email'  => '<div><label for="email" class="block text-sm font-medium text-dark mb-2">Email' . ( $req ? ' *' : '' ) . '</label><input id="email" name="email" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors" placeholder="tu@email.com"' . $aria_req . ' /></div></div>',
			'url'    => '<div class="mb-4"><label for="url" class="block text-sm font-medium text-dark mb-2">Sitio web (opcional)</label><input id="url" name="url" type="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition-colors" placeholder="https://tusitio.com" /></div>',
		),
		'class_submit'        => 'submit-button',
		'comment_notes_before' => '<p class="text-sm text-gray-600 mb-4">Tu dirección de email no será publicada. Los campos obligatorios están marcados con *</p>',
		'comment_notes_after'  => '',
	);
}
